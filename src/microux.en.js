function Microux() {
  ...
  this.uiconnection = function(){
    ...
    if(this.isconnection && messageText.classList == 'online_status--off'){
      this.uiMessage('You\'ve connected to the internet again.', 'online_status--on online_status--again', icon);
    }else if(!this.isconnection){
      //unconnection
      this.uiMessage('It seems like you lost internet connection.', 'online_status--off', icon);
    }
  };
    ...
    battery.then(function(battery){
      ...
      function updateLevel(){
        if((battery.level*100) < 100 && !battery.charging){
          ...
          vmMicro.uiMessage('It seems that your battery level is low.', 'battery_status--off', icon);
        }
      }
    });
  }
