function Microux() {
  ...
  this.uiconnection = function(){
    ...
    if(this.isconnection && messageText.classList == 'online_status--off'){
      this.uiMessage('Você está conectado novamente.', 'online_status--on notification_container', icon);
    }else if(!this.isconnection){
      //unconnection
      this.uiMessage('Parece que você perdeu sua conexão.', 'online_status--off notification_container', icon);
    }
  };
    ...
    battery.then(function(battery){
      ...
      function updateLevel(){
        if((battery.level*100) < 30 && !battery.charging){
          ...
          vmMicro.uiMessage('Parece que sua bateria está fraca.', 'battery_status--off notification_container', icon);
        }
      }
    });
  }
